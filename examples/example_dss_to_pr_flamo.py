...
def _(mo):
    ...
    **In-depth math documentation** of the refinement fix, with an **SOS filter in the loop** and plotting.
    ...
    1. Build a delay state-space model with a **first-order absorption shelf in the loop** via `fdn_build_gallery(..., rt=..., rt_nyquist=...)` and `build_to_flamo`. The omitted crossover is the shelf midpoint `fs/4`.
...
def _(pyFDN, torch):
    ...
    build = pyFDN.fdn_build_gallery(
        ...
        rt_nyquist=0.1,  # ... and at Nyquist; the shelf midpoint is fs/4
        rng=7,
    )
